...
if selected:
    ...
    riker = {"Criteria": ["Height", "Weight", "Rushing Efficiency", "Rushing Explosiveness", "Receiving Efficiency",
             ...
             "Notes": ["as listed by school", "as listed by school",
                       ...
                       "Capped at 20.", "Max of Receiving Efficiency & Explosiveness", "per 247 Sports",
                       "per Bill Connelly",
    ...
    if Rush_Eff:
        rb_comp_df['comp_rusheff_comp'] = rb_comp_df['Rushing Efficiency'] - playerSZN_comp_df[
            'Rushing Efficiency'].max()
    ...
    if Rush_Exp:
        rb_comp_df['comp_rushexp_comp'] = rb_comp_df['Rushing Explosiveness'] - playerSZN_comp_df[
            'Rushing Explosiveness'].max()
    ...
    if Pass_Eff:
        rb_comp_df['comp_passeff_comp'] = rb_comp_df['Receiving Efficiency'] - playerSZN_comp_df[
            'Receiving Efficiency'].max()
    ...
    if Pass_Exp:
        rb_comp_df['comp_passexp_comp'] = rb_comp_df['Receiving Explosiveness'] - playerSZN_comp_df[
            'Receiving Explosiveness'].max()
    ...
    formatter2 = {
        ...
        'Receiving Efficiency', {'width': 70, 'cellStyle': agstyler.highlight(Color.GREEN_LIGHT.value,
                                                                              condition_one_value)}),
        'Rushing Explosiveness': (
        'Rushing Explosiveness', {'width': 70, 'cellStyle': agstyler.highlight(Color.GREEN_LIGHT.value,
                                                                               condition_one_value)}),
        'Receiving Explosiveness': (
        'Receiving Explosiveness', {'width': 70, 'cellStyle': agstyler.highlight(Color.GREEN_LIGHT.value,
                                                                                 condition_one_value)}),
    ...
    if nfl_only:
        ...
        nfl_comp2 = nfl_comp2[
            ['Name', 'Comp Score', 'Team', 'Season', 'Games Played', 'Height', 'Weight', 'Rushing Efficiency',
    ...
    data2 = draw_grid(
        nfl_comp2.loc[nfl_comp2['Games Played'] >= 4].sort_values(by=['Comp Score'], ascending=False).round(
            decimals=1).head(row_number),
    ...
    def interactivePlot2():
        ...
        plot.add_trace(px.scatter(messy.loc[messy['NFL Draft Pick'] > 0].dropna(), x='Year from Highschool', y=stat,
                                  template='simple_white').update_layout(plot_bgcolor="rgb(0,0,0)",
                                                                         paper_bgcolor="rgb(0,0,0)").update_traces(
            marker={
                'size': 2,
                'color': 'gray'}).data[0])
    ...
    st.text("Select Season to Continue")
